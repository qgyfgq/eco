...
def mean_information_coefficient(factor_data,
    ...
    if by_time is not None:
        ...
        grouper.append(pd.Grouper(freq=by_time.replace('M', 'ME')))
    if by_group:
        grouper.append('group')
...
def mean_return_by_quantile(factor_data,
    ...
    group_stats = factor_data.groupby(grouper, group_keys=False)[
        utils.get_forward_returns_columns(factor_data.columns)] \
        .agg(['mean', 'std', 'count'])
    ...
    if not by_date:
        ...
        group_stats = mean_ret.groupby(grouper, group_keys=False)\
            .agg(['mean', 'std', 'count'])
        mean_ret = group_stats.T.xs('mean', level=1).T
...
def create_pyfolio_input(factor_data,
    ...
    returns = cumrets.pct_change(fill_method=None).fillna(0)
    ...
    if benchmark_period in fwd_ret_cols:
        ...
        benchmark_rets = benchmark_rets.pct_change(fill_method=None).fillna(0)
